...
class UsTaxReturn(models.Model):
    ...
    def action_create_remittance(self):
        ...
        # The remittance bill debits the payable by the return total; if that
        # doesn't match the tax actually collected in the GL, the reconcile would
        # strand a residual. Make the operator clear the discrepancy first.
        if self.tie_out_variance:
            raise UserError(
                self.env._(
                    "The return total does not tie out to the tax collected in "
                    "the GL (variance %(var)s). Reconcile the payable account for "
                    "%(state)s before remitting.",
                    var=self.tie_out_variance,
        ...
        partner = authority.partner_id or company.us_tax_authority_partner_id
