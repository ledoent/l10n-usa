...
# ``base`` = jurisdiction levels the allowance is computed on: FL & PA apply it to
# the STATE-level tax only (not local surtax); TX (base=None) to the total due.
# Rates/caps are best-effort and should be confirmed against current state rules.
ALLOWANCE = {
    "FL": {
        "rate": 0.025,
        "cap": 30.0,
        "base": ("state",),
        "label": "Collection Allowance (2.5% of state tax, max $30)",
    },
    "TX": {
        "rate": 0.005,
        "cap": None,
        "base": None,
        "label": "Timely Filing Discount (0.5%)",
    },
    "PA": {
        "rate": 0.01,
        "cap": 25.0,
        "base": ("state",),
        "label": "Vendor Discount (1% of state tax, monthly cap $25)",
    },
}
FORM_NAME = {
...
def collection_allowance(record):
    ...
    if not cfg:
        return 0.0
    base = _tax(record, cfg["base"]) if cfg["base"] else record.total_tax
    if base <= 0:
        # No allowance on a zero or net-refund period.
        return 0.0
    amount = base * cfg["rate"]
