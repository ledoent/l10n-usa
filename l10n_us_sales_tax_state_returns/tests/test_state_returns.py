...
class TestStateReturns(TransactionCase):
    ...
    def test_fl_worksheet_has_surtax_section(self):
        ...
        self.assertIn("Collection Allowance (2.5% of state tax, max $30)", csv_text)

    def test_allowance_uses_state_base_not_local(self):
        # FL/PA allowance is on the STATE tax only, not the local surtax.
        fl = self._make_return("FL", 400.0, locals_=[("Dade", "086", 5000.0, 500.0)])
        # 2.5% of state 400 = 10.0 (NOT 2.5% of total 900 = 22.5).
        self.assertAlmostEqual(
            state_return_builder.collection_allowance(fl), 10.0, places=2
        )

    def test_no_allowance_on_refund_period(self):
        # A net-refund period earns no collection allowance.
        fl = self._make_return("FL", -200.0)
        self.assertEqual(state_return_builder.collection_allowance(fl), 0.0)
