...
class UsTaxNexus(models.Model):
    ...
    def get_single_local_rate(self, company_id, state_id):
        """The single/simplified local rate for this company+state, else None.

        Returns the decimal rate when it applies (elected or mandatory) and is
        non-zero; the caller has confirmed the sale is interstate (remote).
        """
        nexus = self._single_local_nexus(company_id, state_id)
        return nexus.single_local_rate if nexus else None
