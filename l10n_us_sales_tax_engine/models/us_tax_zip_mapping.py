...
class UsTaxZipMapping(models.Model):
    ...
    def learn_jurisdiction(self, address, jurisdiction, source="api"):
        ...
        # System-managed cache: the engine runs as the acting (often read-only)
        # user, so sudo the write or normal users AccessError / never learn.
        Mapping = self.sudo()
        existing = Mapping.search([("address_key", "=", key)], limit=1)
        if existing:
            existing.write(vals)
            return existing
