...
class TestProviderRegistry(UsTaxBaseTest):
    ...
    def test_registry_lists_base_providers(self):
        ...
        self.assertTrue({"local", "ziptax", "api_ninjas", "taxjar"} <= set(classes))

    def test_get_service_returns_class(self):
        self.assertIs(self._provider("ziptax")._get_provider_service(), ProviderZipTax)
        self.assertIs(self._provider("taxjar")._get_provider_service(), ProviderTaxJar)
    ...
    def test_taxjar_uses_lowercase_name_keys(self):
        svc = ProviderTaxJar(self._provider("taxjar"))
        normalized = {
            "state_rate": 0.0,
            "county_rate": 0.01,
            "city_rate": 0.005,
