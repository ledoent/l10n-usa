...
class UsTaxEngineService(models.AbstractModel):
    ...
    def _apply_to_sale_order(self, order, result):
        ...
        # Order-wide evaluated exemption — no nexus in this state, or a valid
        # customer certificate: assign the explicit 0% exempt tax on every line.
        if order_source in ("exempt_nexus", "exempt_customer"):
    ...
    def _apply_to_invoice(self, move, result):
        ...
        if move_source in ("exempt_nexus", "exempt_customer"):
    ...
    def _get_or_create_component_tax(self, state_code, state, company, comp, tax_group):
        ...
        if not tax and jurisdiction_id:
            ...
            tax = Tax.search(
                [("name", "=", alt_name)] + domain[1:], limit=1
            )
    ...
    def _get_or_create_group_tax(
        ...
        composition = ", ".join(
            f"{c.amount:.4g}" for c in children
        )
        for name in (
            base_name,
